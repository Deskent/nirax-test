...
import FirmAcc from './firm.acc.component.jsx';
...
export default function Firm ({type}) {
    ...
    useEffect(() => {
        ...
        if(type==="edit") {
            setTitle("Редактирование компании");
            FirmService.get(params.id).then(
                response => {
                ...
                const tempDataItems = response.data.data.map((item, i) => {
                    return (
                        <tr key={i} onDoubleClick={() => onEdit(item.id)} onKeyUp={(e) => {if (e.keyCode === 13) { onEdit(item.id)}}} tabIndex={0}>
                            <td>{item.form_reg==='1'? item.f_fio:item.f_name}</td>
                            <td>{item.f_inn}</td>
                            <td>
                                ...
                                <button
                                    ...
                                    data-tooltip-id="button-tooltip"
                                    data-tooltip-content="Удалить компанию"
                                    data-tooltip-place="right">
                                    <img src={deletePng} alt="удалить компанию"/>
                                </button>
                            </td>
                        </tr>
                    )
    ...
    const onEdit = (id) => {
        navigate(`${config._install_path}firm/${id}`);
    }

    const goBack = () => {
        navigate(`${config._install_path}firm`);
    }

    const addNewUser = () => {
        navigate(`${config._install_path}firm/add`);
    }
        ...
        if(validate())
        {
            if(params.id)
            {
                FirmService.update(
                    ...
                    response => {
                       // if(response.data) navigate(`${config._install_path}firm`);
                    },
                    error => {
                        setError(error, setFormErrors);
                    }
            ...
            else
            {
                FirmService.add(
                    ...
                    response => {
                        if(response.data.user) navigate(`${config._install_path}firm/${response.data.user.id}`);
                    },
                    error => {
                        setError(error, setFormErrors);
                    }
                ...
                {type==="add"||type==="edit"?
                    (
                        <>
                            ...
                            <div>
                                <Button onClick={saveFirm}>Сохранить</Button>
                            </div>
                            {params.id? <FirmAcc firmId={params.id}/>: null}
                        </>
                    ):
                    (
                         <>
